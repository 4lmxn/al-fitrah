          ...
          {staff.map((s) => (
            <li key={s.id} className="rounded-2xl border border-emerald/10 bg-white/90 p-5 shadow-soft">
              ...
              {!isOwner && (s.access || s.email) ? (
                <p className="mt-4 border-t border-emerald/10 pt-4 text-xs text-ink/70">
                  {s.access
              ...
              <ActionForm action={updateStaff} className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <button
                    type="submit"
                    className="min-h-11 rounded-full bg-white px-4 py-2.5 text-sm font-semibold text-emerald-deep ring-1 ring-emerald/15 transition hover:bg-emerald/5"
                  >
                    Save details
                  </button>
                </div>
              ...
              {isOwner ? (
                <div className="mt-4 flex flex-wrap items-end gap-2 border-t border-emerald/10 pt-4">
                  <ActionForm action={setStaffAccess} className="flex flex-wrap items-end gap-2">
                    ...
                    <button
                      type="submit"
                      className="min-h-11 rounded-full bg-emerald px-5 py-2.5 text-sm font-semibold text-cream transition hover:bg-emerald-deep"
                    >
                      Apply
                    </button>
                  </ActionForm>
              ...
              {isOwner && (
                <div className="mt-3 border-t border-emerald/10 pt-3">
                  <ActionForm
                    action={deleteStaff}
                    confirm={`Remove ${s.name} from the staff list? This cannot be undone. Their attendance history is kept.`}
                  >
                    <input type="hidden" name="id" value={s.id} />
                    <button
                      type="submit"
