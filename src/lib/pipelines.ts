...
export async function getPipeline(type: LeadType): Promise<StageView[]> {
  ...
  // Position among ACTIVE stages only — that is what deepens the colour through
  // the middle of the pipeline regardless of how many stages a school defines.
  let activeSeen = -1;
  return stages.map((s) => {
...
/** Terminal ids across every pipeline — for queries that span both types. */
